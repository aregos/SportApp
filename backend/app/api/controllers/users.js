...
module.exports = {
  ...
  updateSettingsList: function(req, res, next) {
    userModel.updateOne(
      { login: req.body.login },
      { settingsList: req.body.settingsList },
      function(err, result) {
        if (err) {
          res.status(500).json({
            error: err,
            message:
              "Не найден пользователь или не удалось обновить список настроек"
          });
          next(err);
        } else {
  ...
  getSettingsList: function(req, res, next) {
    if (req.body.login) {
      userModel.findOne({ login: req.body.login }, function(err, result) {
        if (err) {
          res.status(500).json({
            error: err,
            message:
              "Не удалось получить данные настроек пользователя, загружаем стандартные"
          });
          next(err);
        } else {
      ...
      res
        .status(500)
        .json({ message: "Не удалось получить логин пользователя" });
  },
  ...
  searchFriends: function(req, res, next) {
    if (req.body.name) {
      userModel.find({ name: req.body.name }, function(err, result) {
        if (err) {
          res.status(500).json({
            message: `не найдено пользователей с именем ${req.body.name}`
          });
          next(err);
        } else {
  ...
  sendFriendRequest: function(req, res, next) {
    userModel.findById(req.body.id, function(err, result) {
      if (err) {
        res
          .status(500)
          .json({ error: err, message: "Не удалось найти профиль" });
        next(err);
      } else {
        ...
        if (friendsOutRequests.includes(req.body.friendId)) {
          res
            .status(500)
            .json({ message: "Вы уже отправили заявку этому пользователю" });
        } else {
          userModel.findOneAndUpdate(
            { _id: req.body.id },
            { friendsOutRequests: req.body.friendId },
            (err, result) => {
              if (err) {
                res.status(500).json({
                  error: err,
                  message: "Произошла ошибка при попытке добавления в друзья"
                });
              } else {
                userModel.findOneAndUpdate(
                  { _id: req.body.friendId },
                  { friendsInRequests: req.body.id },
                  function(err, result) {
                    if (err) {
                      res.status(500).json({
                        err,
                        message:
                          "Произошла ошибка при попытке добавления в друзья"
                      });
                    } else {
                      res.status(200).json({
  ...
  acceptFriendRequest: async function(req, res, next) {
    const userUpdate = userModel.findByIdAndUpdate(
      req.body.id,
      {
        $addToSet: { friends: { id: req.body.friendId } },
        $pull: { friendsInRequests: req.body.friendId }
      },
      function(err, result) {
        if (err) {
          res
            .status(500)
            .json({ error: err, message: "Не удалось принять заявку" });
        }
      }
    );
    const friendUpdate = userModel.findByIdAndUpdate(
      req.body.friendId,
      { $pull: { friendsOutRequests: req.body.id } },
      (err, result) => {
        if (err) {
          res.status(500).json({
            error: err,
